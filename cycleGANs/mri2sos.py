...
def training(epochs, G_A2B, G_B2A, D_A, D_B, optimizer_G_A2B, optimizer_G_B2A, optimizer_D_A, optimizer_D_B, dataloader_mri, dataloader_sos, old=True):
    ...
    for epoch in range(epochs):
        old_a_fake = torch.Tensor()
        old_b_fake = torch.Tensor()
        ...
        save_models(G_A2B, G_B2A, D_A, D_B)
        if(epoch % 5 == 0):
...
if __name__ ==  '__main__':
    ...
    criterion_Im = torch.nn.MSELoss() # torch.nn.L1Loss()
