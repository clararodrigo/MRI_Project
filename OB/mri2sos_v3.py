...
def init_dataset(bs, test=False):
    if(test == False): path = '/home/oab18/Projects/MRI_Project/Dataset/noisy_4/train/'
    if(test == True): path = '/home/oab18/Projects/MRI_Project/Dataset/noisy_4/test/'
    dataset_mri = torch.load(path+'mri_dataset.pt')
    dataset_sos = torch.load(path+'sos_dataset.pt')

    workers = 2
    image_size = (64,64)
    dataroot = r'/home/oab18/Projects/MRI_Project/Dataset/noisy_4/'
    ...
    dataloader_sos = torch.utils.data.DataLoader(dataset_sos, batch_size=bs,
                                            shuffle=True, num_workers=2)
...
def train(epochs, G_A2B, G_B2A, D_A, D_B, optimizer_G_A2B, optimizer_G_B2A, optimizer_D_A, optimizer_D_B, dataloader_mri, dataloader_sos, old=True):
  ...
  for epoch in range(epochs):
    ...
    for i,(data_mri, data_sos) in enumerate(zip(dataloader_mri, dataloader_sos),0):
        ...
        b_real = data_sos[0].unsqueeze(0).unsqueeze(0).to(device)
    ...
    if (epoch % 5 == 0):
      plot_images_test(dataloader_mri, dataloader_sos, epoch)
...
if __name__ ==  '__main__':
    ...
    test_dataloader_mri, test_dataloader_sos = init_dataset(1, True)
